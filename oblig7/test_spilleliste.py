...
class TestSpilleliste(unittest.TestCase):
    ...
    @unittest.skipUnless(
        __finn_sang_tittel and __legg_til_sang and __fjern_sang, _standard_skip_melding
    )
    ...
    def test_spill_alle(self):
        ...
        for tittel, artist, sang in self._testsanger:
            self.assertIn(
                artist,
                output,
                msg=f"Artist '{artist}' ble ikke printet for sangen '{tittel}' av '{artist}'",
            )
            self.assertIn(
                tittel,
                output,
                msg=f"Tittel '{tittel}' ble ikke printet for sangen '{tittel}' av '{artist}'",
            )
    ...
    def test_les_fra_fil(self):
        ...
        for tittel, artist, _ in self._testsanger:
            ...
            self.assertIsNotNone(
                retur, f"Fant ikke '{tittel}' av '{artist}' etter å ha lest fra fil"
            )

    @unittest.skipUnless(
        __legg_til_sang and __hent_artist_utvalg, _standard_skip_melding
    )
    def test_hent_artist_utvalg(self):
        ...
        lokale_testsanger = [
            (
                tittel := "Shake That",
                artist := "Eminem and Nate Dogg",
                Sang(tittel, artist),
            ),
            (tittel := "Mockingbird", artist := "Eminem", Sang(tittel, artist)),
            (
                tittel := "Nobody Does It Better",
                artist := "Nate Dogg",
                Sang(tittel, artist),
            ),
        ]
        ...
        alle_utvalg = {
            artist: self._spilleliste.hent_artist_utvalg(artist)
            for artist in alle_artister
        }
            ...
            with open(gammel_musikk_fil, "r") as gammel:
                ...
                for linje in gammel:
                    self.assertIn(
                        linje,
                        nye_linjer,
                        msg=f"Forventet linjen '{linje}' i den nye filen, men fant den ikke",
                    )
