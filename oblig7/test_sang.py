...
def generer_test_spill(artist, tittel):
    ...
    test_spill.__doc__ = (
        f"Tester om .spill() printer sangen som spilles ({tittel} av {artist})"
    )
    return test_spill
...
def generer_test_sjekk_artist(artist_under_sjekk, artist, tittel, forventet_retur):
    def test_sjekk_artist(self):
        ...
        self.assertEqual(
            sang.sjekk_artist(artist_under_sjekk), forventet_retur, msg=error_msg
        )

    test_sjekk_artist.__name__ = lag_parametrisert_test_navn(
        test_sjekk_artist, artist_under_sjekk, forventet_retur
    )
...
def generer_test_sjekk_tittel(tittel_under_sjekk, artist, tittel, forventet_retur):
    def test_sjekk_tittel(self):
        ...
        self.assertEqual(
            sang.sjekk_tittel(tittel_under_sjekk), forventet_retur, msg=error_msg
        )

    test_sjekk_tittel.__name__ = lag_parametrisert_test_navn(
        test_sjekk_tittel, tittel_under_sjekk, forventet_retur
    )
...
def generer_test_sjekk_artist_og_tittel(
    sjekket_tittel, sjekket_artist, artist, tittel, forventet_retur
):
    def test_sjekk_artist_og_tittel(self):
        ...
        self.assertEqual(
            sang.sjekk_artist_og_tittel(sjekket_artist, sjekket_tittel),
            forventet_retur,
            msg=error_msg,
        )

    test_sjekk_artist_og_tittel.__name__ = lag_parametrisert_test_navn(
        test_sjekk_artist_og_tittel, sjekket_artist, sjekket_tittel, forventet_retur
    )
...
def generer_test_streng_til_fil(artist, tittel, forventet_retur):
    ...
    test_streng_til_fil.__name__ = lag_parametrisert_test_navn(
        test_streng_til_fil, forventet_retur
    )
    test_streng_til_fil.__doc__ = (
        f"Tester at .streng_til_fil() returnerer {forventet_retur.rstrip()} for sangen '{tittel}' av '{artist}'"
        ""
    )
...
def __legg_til_tester_i_TestSang():
    ...
    spill_parametre = [["Cryoshell", "Bye Bye Babylon"], ["Linkin Park", "In the End"]]

    sjekk_artist_parametre = [
        [
            "Snoop",
            artist := "Dr. Dre and Snoop Dogg",
            tittel := "The Next Episode",
            True,
        ],
    ...
    genererte_tester = _generer_tester(
        ...
        (
            generer_test_sjekk_artist_og_tittel,
            sjekk_artist_og_tittel_parametre,
            __sjekk_artist_og_tittel,
        ),
